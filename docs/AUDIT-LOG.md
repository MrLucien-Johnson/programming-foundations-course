...
**Gaps found**
1. Guest progress soft-gate clarified: stored until refused (banner still asks for a clear choice).
...
4. Mark-complete / persona could appear to succeed when persistence was blocked (after refuse).
...
**Fixes**
- Guest persistence continues until `denied`; dismiss notice from Privacy; push empty cloud progress on reset.
- Honest blocked-persist messaging when the learner has refused on-device storage.
- Consent banner layout CSS; shared footer essential links; polished advanced course hubs.
- Escape/link close for mobile nav; empty-quiz guard; tutorial panel a11y; cache `?v=ux32` / `ux33`.
