# AI Foundations (Beginner)

Build confidence with prompt design and safe usage.

## Learning Outcomes
- Structure prompts with roles, constraints, and iteration notes.
- Demonstrate safe refuse/redirect behavior on risky inputs.
- Keep a prompt journal showing what improved and what did not.

## Deliverables
- prompt journal with iterations
- examples showing safe behavior

## Verification Checklist
Before moving on, confirm the following:

- You can restate the module goal in your own words.
- You have run at least one evaluation pass.
- You documented what improved and what did not.

## Guided Walkthrough
Follow these steps to turn the lesson into a real, working deliverable.

1. Copy the starter pack from `languages/ai/beginner/starter-pack` into a new working folder.
2. Review the module goals and plan how you will deliver the module goal with confidence.
3. Review the module goals and define a clear deliverable.
4. Implement the core feature in the starter pack.
5. Add tests and run the verification checklist.
6. Document decisions in a short README section (assumptions, tradeoffs, next steps).

## Starter Pack

Use the starter pack in `languages/ai/beginner/starter-pack` for a clean baseline.

Inside the pack:
- A prompt spec template.
- An evaluation notes template.
- A place to capture workflow decisions.

