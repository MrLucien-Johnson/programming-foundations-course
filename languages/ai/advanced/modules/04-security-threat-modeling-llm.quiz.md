# AI — Module 04: Security Threat Modeling for LLM Apps Quiz: Test Your Understanding

## 📝 Instructions

Answer these questions about what you learned. Try to answer from memory first!

## 🧪 Questions

### Question 1: A hiring manager asks what you can now do confidently. Which outcome fits?
A) Add a least-privilege tool policy and verify via tests.
B) Design incident response for security events (triage + containment).
C) Apply safety and policy basics: refuse/redirect, handle sensitive data, reduce hallucinations.
D) Create a threat model for one app: list threats and mitigations.

**Your answer:** _______________

---

### Question 2: Which requirement belongs in the testing checklist for this module?
A) Security tests included in the evaluation harness and CI gate.
B) Design a red-team suite that targets top threats.
C) Add data minimization and retention rules to the design.
D) Over-permissioned tools without audit logs.

**Your answer:** _______________

---

### Question 3: Which testing requirement should be verified in CI for this module?
A) Add data minimization and retention rules to the design.
B) Apply safety and policy basics: refuse/redirect, handle sensitive data, reduce hallucinations.
C) Add a supply-chain policy for prompt/eval artifacts.
D) Security tests included in the evaluation harness and CI gate.

**Your answer:** _______________

---

### Question 4: Which Better action best demonstrates stronger engineering discipline?
A) Apply safety and policy basics: refuse/redirect, handle sensitive data, reduce hallucinations.
B) Over-permissioned tools without audit logs.
C) Add a least-privilege tool policy and verify via tests.
D) Build a small evaluation loop: test cases, metrics, and iteration notes.

**Your answer:** _______________

---

### Question 5: A PR introduces new behavior. Which Core action is the minimum expected before review?
A) Create a threat model for one app: list threats and mitigations.
B) Document and version changes so improvements are explainable.
C) Includes tool security, data security, and injection defenses.
D) Produce prompts/specs that are repeatable (not “one-off magic prompts”).

**Your answer:** _______________

---

### Question 6: A reviewer is ready to approve once one missing requirement is fixed. Which requirement is it?
A) Security tests included in the evaluation harness and CI gate.
B) Includes tool security, data security, and injection defenses.
C) Design incident response for security events (triage + containment).
D) Treating retrieved content as trusted instructions.

**Your answer:** _______________

---

### Question 7: A reviewer says, 'Good start.' Which Better upgrade should you add next?
A) Translate a vague request into a clear goal, constraints, and success criteria.
B) Document and version changes so improvements are explainable.
C) Add data minimization and retention rules to the design.
D) Design incident response for security events (triage + containment).

**Your answer:** _______________

---

### Question 8: You have extra time to go beyond expectations. Which Beast Mode action best shows senior-level rigor?
A) Create a threat model for one app: list threats and mitigations.
B) Includes tool security, data security, and injection defenses.
C) Add a supply-chain policy for prompt/eval artifacts.
D) Design a red-team suite that targets top threats.

**Your answer:** _______________

---

### Question 9: Which behavior is listed as a common mistake for this module?
A) Treating retrieved content as trusted instructions.
B) Design incident response for security events (triage + containment).
C) Build a small evaluation loop: test cases, metrics, and iteration notes.
D) Security tests included in the evaluation harness and CI gate.

**Your answer:** _______________

---

### Question 10: Which Core action would a senior engineer insist on before approving the change?
A) Add data minimization and retention rules to the design.
B) Security tests included in the evaluation harness and CI gate.
C) Add a supply-chain policy for prompt/eval artifacts.
D) Design a red-team suite that targets top threats.

**Your answer:** _______________

---

## ✅ Check Your Answers

Once you finish, check the answers file for explanations.

## 🎯 How Did You Do?

- **10/10 correct:** Excellent! You understand the module well. 🎉
- **8-9/10 correct:** Great work! Review the ones you missed.
- **0-7/10 correct:** Review the module and try again. 💪
