# SQL (PostgreSQL) Intermediate — Module 07: Debugging and Performance

## Overview
Debug systematically and improve performance with evidence.
...
- Able to run a local project in this environment: **PostgreSQL 15+ (local via Docker)**.
- Basic CLI literacy (running tests, reading logs).
- For modules that involve data: a local **PostgreSQL** instance (recommended via Docker + docker-compose to run Postgres).

## Lessons
1) Debugging workflow: reproduce → isolate → fix (35 min)
2) Logging and tracing basics (40 min)
3) Profiling CPU + memory (45 min)
4) Database performance: slow queries + indexes (45 min)

## Guided Walkthrough

Follow these steps to turn the lesson into a real, working deliverable.

1. Copy the starter pack from `languages/sql/intermediate/starter-pack` into a new working folder.
2. Review the module goals and plan how you will diagnose and fix slow or failing code.
3. Capture a baseline timing or log snapshot.
4. Use a profiler or debug tool to identify the bottleneck.
5. Implement one optimization and measure the result.
6. Document decisions in a short README section (assumptions, tradeoffs, next steps).

## Starter Pack

Use the starter pack in `languages/sql/intermediate/starter-pack` for a clean baseline.
...
## Mini-Project
### Brief
Build a small, production-leaning feature or service slice that showcases **Debugging and Performance** in a way you could explain in a code review.
...
- All work must be covered by **migrations apply cleanly + pgTAP + SQL linting in CI**.
...
- Performance checklist: https://developers.google.com/web/fundamentals/performance
- Profiling basics: https://betterprogramming.pub/a-guide-to-code-profiling-3546b2fcd76a
